...
function BarSparkline(props: { values?: number[]; tone: StatCardTone }) {
  ...
  const barKeyCounts = new Map<string, number>()
  const bars = sparkline.map((height) => {
    const heightKey = height.toFixed(4)
    const count = barKeyCounts.get(heightKey) ?? 0
    barKeyCounts.set(heightKey, count + 1)

    return {
      height,
      key: `spark-${heightKey}-${count}`,
    }
  })
  ...
  return (
    <div className='grid grid-cols-2 gap-2'>
      {props.details.map((detail) => (
        <div
          key={detail.label}
          className='bg-muted/40 min-w-0 rounded-lg border border-transparent px-3 py-2'
        >
...
export function StatCard(props: StatCardProps) {
  ...
  let valueContent = (
    <div className='flex flex-col gap-1'>
      <div className='text-foreground font-mono text-[1.35rem] leading-tight font-semibold tracking-tight break-all tabular-nums sm:text-2xl'>
        {props.value}
      </div>
  ...
  let footerContent = <BarSparkline values={props.sparkline} tone={tone} />
  if (props.details?.length) {
    footerContent = <StatCardDetails details={props.details} />
  } else if (sparklineVariant === 'line') {
    footerContent = <LineSparkline values={props.sparkline} tone={tone} />
  }

  return (
    <div className='group flex min-h-40 flex-col justify-between gap-3'>
