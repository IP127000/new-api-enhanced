...
4. **缓存与性能指标可观测性**：使用日志展示缓存命中 token 与命中率；概览展示全量历史 Token、消耗、请求和性能健康；数据看板按所选统计区间展示输入、输出、缓存、命中率、费用和请求次数。
5. **管理后台体验优化**：优化概览页用量卡片、性能健康和模型排行布局；渠道编辑页支持管理员直接显示已保存密钥，减少重复验证对运维排障的阻塞。
6. **排名页统计修复**：排名页改为从消费日志中的 prompt/completion token 统计模型用量，普通用户只查看自己的使用排名，管理员查看全局排名，并通过独立缓存避免不同权限范围互相串数据。
...
- 概览页的性能健康为全量历史统计，数据看板页的性能健康跟随当前统计区间，两者统计范围不同。
- 排名页普通用户展示本人使用范围，管理员展示全局范围；如果排查排名异常，请优先检查消费日志中的 prompt/completion token 字段。
