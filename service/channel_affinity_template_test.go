...
func TestChannelAffinityHitCodexTemplatePassHeadersEffective(t *testing.T) {
	...
	require.Equal(t, "desktop-sess-123", info.RuntimeHeadersOverride["session-id"])
...
func TestChannelAffinityCodexTemplateSyncsPromptCacheKeyToSessionHeader(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	affinityValue := fmt.Sprintf("pc-app-%d", time.Now().UnixNano())
	rec := httptest.NewRecorder()
	ctx, _ := gin.CreateTestContext(rec)
	ctx.Request = httptest.NewRequest(http.MethodPost, "/v1/responses", strings.NewReader(fmt.Sprintf(`{"prompt_cache_key":"%s"}`, affinityValue)))
	ctx.Request.Header.Set("Content-Type", "application/json")
	...
	out, err := relaycommon.ApplyParamOverrideWithRelayInfo([]byte(fmt.Sprintf(`{"model":"gpt-5","prompt_cache_key":"%s"}`, affinityValue)), info)
	require.NoError(t, err)
	require.JSONEq(t, fmt.Sprintf(`{"model":"gpt-5","prompt_cache_key":"%s"}`, affinityValue), string(out))
	require.True(t, info.UseRuntimeHeadersOverride)
	require.Equal(t, affinityValue, info.RuntimeHeadersOverride["session-id"])
	require.Equal(t, affinityValue, info.RuntimeHeadersOverride["session_id"])
	require.Equal(t, "codex-app-test", info.RuntimeHeadersOverride["user-agent"])
}

func TestChannelAffinityCodexTemplatePassesDesktopSessionId(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	affinityValue := fmt.Sprintf("pc-desktop-%d", time.Now().UnixNano())
	rec := httptest.NewRecorder()
	ctx, _ := gin.CreateTestContext(rec)
	ctx.Request = httptest.NewRequest(http.MethodPost, "/v1/responses", strings.NewReader(fmt.Sprintf(`{"prompt_cache_key":"%s"}`, affinityValue)))
	ctx.Request.Header.Set("Content-Type", "application/json")
	...
	info := &relaycommon.RelayInfo{
		RequestHeaders: map[string]string{
			...
			"Session-Id": "desktop-session-123",
			"User-Agent": "codex-app-test",
		},
		ChannelMeta: &relaycommon.ChannelMeta{
			ParamOverride: mergedOverride,
	...
	require.JSONEq(t, fmt.Sprintf(`{"model":"gpt-5","prompt_cache_key":"%s"}`, affinityValue), string(out))
	require.True(t, info.UseRuntimeHeadersOverride)
	require.Equal(t, "Codex Desktop", info.RuntimeHeadersOverride["originator"])
	require.Equal(t, "desktop-session-123", info.RuntimeHeadersOverride["session-id"])
	require.Equal(t, affinityValue, info.RuntimeHeadersOverride["session_id"])
	require.Equal(t, "codex-app-test", info.RuntimeHeadersOverride["user-agent"])
}
