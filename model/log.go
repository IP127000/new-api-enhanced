...
func shouldPersistLogType(logType int) bool {
	switch logType {
	case LogTypeConsume, LogTypeTopup, LogTypeRefund:
		return true
	default:
		return false
	}
...
func RecordLog(userId int, logType int, content string) {
	if !shouldPersistLogType(logType) {
		return
	}
...
// RecordLogWithAdminInfo records only persisted log types and stores admin details in Other.admin_info.
...
// RecordLoginLog intentionally skips persistence to keep the logs table focused on billable activity.
func RecordLoginLog(userId int, username string, content string, ip string, action string, params map[string]interface{}, extra map[string]interface{}) {
	return
}

// RecordOperationAuditLog intentionally skips persistence to avoid writing admin/settings changes.
...
func RecordTaskBillingLog(params RecordTaskBillingLogParams) {
	if !shouldPersistLogType(params.LogType) {
		return
	}
...
func GetConsumeLogModelRequestCounts(startTimestamp int64, endTimestamp int64, username string) ([]ModelRequestCount, error) {
...
func sumCacheTokens(tx *gorm.DB) (int, error) {
	rows, err := tx.Select("other").Rows()
	if err != nil {
		return 0, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if common.UnmarshalJsonStr(other, &data) == nil && data.CacheTokens > 0 {
			total += data.CacheTokens
		}
	}
...
func SumUsedQuota(logType int, startTimestamp int64, endTimestamp int64, modelName string, username string, tokenName string, channel int, group string, requestId string, upstreamRequestId string) (stat Stat, err error) {
	...
	if rpmTpmQuery, err = applyLogStatFilters(rpmTpmQuery, startTimestamp, endTimestamp, modelName, username, tokenName, channel, group, requestId, upstreamRequestId); err != nil {
		return stat, err
	}
	...
	cacheTokenQuery := LOG_DB.Table("logs")
	if cacheTokenQuery, err = applyLogStatFilters(cacheTokenQuery, startTimestamp, endTimestamp, modelName, username, tokenName, channel, group, requestId, upstreamRequestId); err != nil {
		return stat, err
	}
	stat.CacheTokens, err = sumCacheTokens(cacheTokenQuery)
	if err != nil {
		common.SysError("failed to query cache token stat: " + err.Error())
		return stat, errors.New("查询统计数据失败")
	}
	if stat.PromptTokens > 0 {
		stat.CacheHitRate = float64(stat.CacheTokens) / float64(stat.PromptTokens)
	}
