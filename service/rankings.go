...
func GetRankingsSnapshot(period string, scope RankingScope) (*RankingsResponse, error) {
	config, err := rankingConfig(period)
	if err != nil {
		return nil, err
	}
	if !scope.Global && scope.UserID <= 0 {
		return nil, fmt.Errorf("rankings scope requires a user")
	}

	now := time.Now()
	cacheKey := rankingCacheKey(config, scope)
	rankingCacheMu.Lock()
	...
	data, err := buildRankingsSnapshot(config, scope, now)
	if err != nil {
		return nil, err
	}
...
func rankingCacheKey(config rankingPeriodConfig, scope RankingScope) string {
	if scope.Global {
		return fmt.Sprintf("%s:global", config.id)
	}
	return fmt.Sprintf("%s:user:%d", config.id, scope.UserID)
}

func rankingScopeLabel(scope RankingScope) string {
	if scope.Global {
		return "global"
	}
	return "self"
...
func rankingScopeUserID(scope RankingScope) int {
	if scope.Global {
		return 0
	}
	return scope.UserID
...
func buildRankingsSnapshot(config rankingPeriodConfig, scope RankingScope, now time.Time) (*RankingsResponse, error) {
	startTime, endTime := rankingTimeRange(config, now)
	userID := rankingScopeUserID(scope)
	currentTotals, err := model.GetRankingQuotaTotals(startTime, endTime, userID)
	if err != nil {
		return nil, err
	}
	currentBuckets, err := model.GetRankingQuotaBuckets(startTime, endTime, config.bucketSize, userID)
	if err != nil {
		return nil, err
	}
	...
	if config.hasPrevious {
		...
		previousTotals, err = model.GetRankingQuotaTotals(previousStart, previousEnd, userID)
		if err != nil {
			return nil, err
		}
	}
