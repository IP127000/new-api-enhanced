...
func buildCodexCliTemplate(headers []string) map[string]interface{} {
	template := buildPassHeaderTemplate(headers)
	operations, _ := template["operations"].([]map[string]interface{})
	template["operations"] = append([]map[string]interface{}{
		{
			"mode": "sync_fields",
	...
	Rules: []ChannelAffinityRule{
		{
			...
			ValueRegex:            "",
			TTLSeconds:            0,
			ParamOverrideTemplate: buildCodexCliTemplate(codexCliPassThroughHeaders),
