...
func TestApplyCodexClientHeaderPassthroughWithRelayInfoUsesBuiltInHeaders(t *testing.T) {
	info := &RelayInfo{
		RequestHeaders: map[string]string{
			"Originator":                            "Codex CLI",
	...
	err := ApplyCodexClientHeaderPassthroughWithRelayInfo(info)
	...
	require.Equal(t, "thread-underscore-123", info.RuntimeHeadersOverride["thread_id"])
	require.Equal(t, "client-request-123", info.RuntimeHeadersOverride["x-client-request-id"])
	require.Equal(t, "turn-state-123", info.RuntimeHeadersOverride["x-codex-turn-state"])
	require.Equal(t, "turn-metadata-123", info.RuntimeHeadersOverride["x-codex-turn-metadata"])
	require.Equal(t, "thread-123:1", info.RuntimeHeadersOverride["x-codex-window-id"])
	require.Equal(t, "memgen-123", info.RuntimeHeadersOverride["x-openai-memgen-request"])
	require.Equal(t, "true", info.RuntimeHeadersOverride["x-responsesapi-include-timing-metrics"])
	require.NotContains(t, info.RuntimeHeadersOverride, "authorization")
}
