...
6. **Rankings accuracy and privacy**: rankings now aggregate model usage from
   consume logs using prompt/completion tokens. Regular users see their own
   usage scope, administrators see global rankings, and separate cache keys
   prevent data from leaking across scopes.
...
- The rankings page is scoped by role: regular users see their own usage
  history, while administrators see global usage. When troubleshooting ranking
  data, check the prompt/completion token fields in consume logs first.
