...
import { CODEX_CLI_HEADER_PASSTHROUGH_JSON } from '../../../../constants/channel-affinity-template.constants';
import {
  IconSave,
  IconClose,
  IconServer,
    ...
    if (name === 'type') {
      ...
      if (value === 57) {
        ...
        if (formApiRef.current) {
          ...
          if (!inputs.param_override?.trim()) {
            formApiRef.current.setValue(
              'param_override',
              CODEX_CLI_HEADER_PASSTHROUGH_JSON,
            );
          }
        }
        setInputs((prev) => ({
          ...prev,
          vertex_files: [],
          param_override: prev.param_override?.trim()
            ? prev.param_override
            : CODEX_CLI_HEADER_PASSTHROUGH_JSON,
        }));
      }
  ...
  const loadChannel = async () => {
    ...
    if (success) {
      ...
      if (
        ...
        data.param_override = CODEX_CLI_HEADER_PASSTHROUGH_JSON;
      }
