# New API Enhanced v1.0.0-enhanced.2-routefix1-dashboard1-keyreveal1-ui1-rankfix2

Rankings accuracy, privacy-scope, and cache isolation fix release.

## Fixes

1. **Rankings token source**: model rankings now aggregate prompt and completion tokens from consume logs instead of relying on stale quota export rows.
2. **User-scope isolation**: regular users now receive rankings for their own usage history only, while administrators continue to receive global rankings.
3. **Scoped rankings cache**: rankings cache keys now include period and scope, preventing global and user-scoped snapshots from sharing cached data.
4. **Rankings freshness**: rankings cache TTL was reduced to 30 seconds so usage changes appear sooner.
5. **Frontend scope awareness**: the rankings page now keys queries by authenticated user/role and adjusts copy for personal versus global ranking views.

## Tests

1. Added model tests for consume-log based ranking totals, bucket aggregation, and per-user filtering.
2. Added service tests for scoped cache keys and missing-user scope validation.
