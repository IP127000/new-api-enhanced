...
func GenRelayInfoCodexWebSearch(c *gin.Context, request *dto.CodexWebSearchRequest) *RelayInfo {
	info := genBaseRelayInfo(c, request)
	info.RelayMode = relayconstant.RelayModeCodexWebSearch
	info.RelayFormat = types.RelayFormatCodexWebSearch
	searchContextSize := strings.TrimSpace(request.Settings.SearchContextSize)
	if searchContextSize == "" {
		searchContextSize = "medium"
	}
	info.ResponsesUsageInfo = &ResponsesUsageInfo{
...
func GenRelayInfo(c *gin.Context, relayFormat types.RelayFormat, request dto.Request, ws *websocket.Conn) (*RelayInfo, error) {
	...
	case types.RelayFormatCodexWebSearch:
		if request, ok := request.(*dto.CodexWebSearchRequest); ok {
			info = GenRelayInfoCodexWebSearch(c, request)
			break
		}
		err = errors.New("request is not a CodexWebSearchRequest")
