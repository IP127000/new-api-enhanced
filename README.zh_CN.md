...
- `codex/upstream-20260704-enhanced`：当前生产/测试功能分支，基于 2026-07-04 上游代码，并包含 Codex 订阅渠道透传、数据看板统计、使用日志统计、概览页恢复等本地改动。
- `back-20260702`：2026-07-02 回滚快照，只作为稳定备份，不在这个分支继续开发。

下一次 AI 或维护者接手前，请先阅读 [RELEASE_NOTES.md](./RELEASE_NOTES.md)。重点如下：

- Codex 订阅渠道只在 `APITypeCodex` + `ChannelTypeCodex` 生效，`/v1/responses` 请求体尽量保持 Codex 客户端原样，同时透传选定的 Codex 客户端请求头。
- 订阅账号身份仍由服务端渠道配置里的 `Authorization` 与 `chatgpt-account-id` 等认证信息决定，不信任客户端认证头。
- `codex cli trace` 渠道亲和规则只用于选择稳定渠道，不再通过 `param_override_template` 修改请求体里的 `prompt_cache_key`。
- 管理员数据看板支持自定义“最近 N 天”，最大值按已有 `quota_data` 最早记录计算，不新增自定义数据库结构。
- 概览页与模型统计卡片需要保留输入 token、输出 token、缓存命中 token、命中率、用量、RPM、TPM 等显示。
- 看板请求次数以成功消费日志 `logs.type = 2` 为准，通过 `/api/log/model-request-counts` 聚合；`perf_metrics` 只作为平均延迟和吞吐量的数据源，不再作为请求次数真源。
- 2026-07-05 countfix 部署使用 `new-api:20260705-countfix` / `new-api-20260705-countfix`，本机端口为 `127.0.0.1:3019`，Caddy 已指向该端口。2026-07-02 回滚镜像/容器保留为 `new-api:20260702-live` / `new-api-20260702-live`。
- 日志与统计清理只保留成功消费、充值、退款/调整日志；失败请求、登录、管理审计、系统设置日志不再持久化到 `logs`。该改动不修改数据库结构。
- 2026-07-04 分支保留了上游的系统信息/健康检查界面和系统任务面板，后续 rebase 不要丢掉这些页面。
- 临时调试分支、请求头日志分支、旧的请求体改写实验分支都不应长期保留。
- 生产统计修正必须在线执行并先做 SQLite 备份。不要为了修复看板计数停止运行容器；当前小时的 `perf_metrics` 内存桶未 flush 前不要直接写入，否则可能重复计数。
- 生产禁忌：绝对不要在调试 SQL、脚本、迁移或看板统计时让生产容器长时间停止。如果确实必须停止，必须先准备好精确的重启和健康检查命令，停止后数秒内立刻启动，确认 `/api/status` 正常后再继续排查。
