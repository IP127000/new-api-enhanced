...
func TestCalculateTextToolCallSurchargeSupportsCurrentWebSearchTool(t *testing.T) {
	t.Parallel()
	...
	summary := &textQuotaSummary{ModelName: "gpt-5.6-sol", GroupRatio: 1}
	...
	require.True(t, surcharge.GreaterThan(decimal.Zero))
}

func TestCalculateCodexWebSearchChargesInputTokensAndOneFixedCall(t *testing.T) {
	recorder := httptest.NewRecorder()
	ctx, _ := gin.CreateTestContext(recorder)
	relayInfo := &relaycommon.RelayInfo{
		OriginModelName: "gpt-5.6-sol",
		RelayFormat:     types.RelayFormatCodexWebSearch,
		PriceData: types.PriceData{
			ModelRatio:      1,
			CompletionRatio: 10,
			GroupRatioInfo:  types.GroupRatioInfo{GroupRatio: 1},
	...
	summary := calculateTextQuotaSummary(ctx, relayInfo, usage)
	require.Equal(t, 20, summary.PromptTokens)
	require.Zero(t, summary.CompletionTokens)
	require.Equal(t, 1, summary.WebSearchCallCount)
	require.Equal(t, 10.0, summary.WebSearchPrice)
	require.Equal(t, 5020, summary.Quota)

	toolOnlySummary := calculateTextQuotaSummary(ctx, relayInfo, &dto.Usage{})
	require.Zero(t, toolOnlySummary.TotalTokens)
	require.Equal(t, 5000, toolOnlySummary.Quota)
	require.True(t, hasBillableTextUsage(toolOnlySummary))
}
